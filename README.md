...
`bind` is backed by a `ContextVar`, so it follows threads and asyncio tasks. Nested blocks merge,
the inner value wins, and each block restores what it found. On a clash, the instance's
